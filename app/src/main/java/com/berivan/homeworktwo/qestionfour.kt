...
fun main() {
    ...
    val coinUinList = ArrayList<CoinUI>()
    for (coinResponse in coinList) {

        val coinUI = CoinUI(
            coinResponse.coinId,
            coinResponse.image,
            coinResponse.description,
            coinResponse.name
        )
        coinUinList.add(coinUI)
    }
    coinUinList.forEach { coinUI ->
        println("coin Id:${coinUI.coinId},coin Name:${coinUI.name},coin description:${coinUI.description},coin image:${coinUI.image}")
